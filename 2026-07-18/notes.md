...
## ★본선 plant 확정: 구경계+완충 8+8 (2026-07-19 밤, 사용자 결정)

용어 정리로 오해 해소: 수정경계="가정"(min(ρ,ρcrit)) / 완충="거리"(실제 셀). 사용자 최종 선택 =
**구경계(zero-gradient 끝)+완충 8+8+ρ180** — 인위적 완화 가정 없이 원래 끝 처리, 혼잡은 실제
셀에서 생성·해소. 검증: 원래 논문 5셀 NC 전부 자연 회복(155 2698/170 3772/skew 3843/incid
4436/190 5732, 순서 정상), **끝 조건 무감**(구경계 vs 표준 Δ0.02% — jam이 끝에 안 닿음 =
경계 논쟁 소멸 영수증). TERM_ZG를 완충 끝에 배선(cf9e05f).

**Phase B(91ed140)**: P-CENT·Wu는 전체 결합 plant 예측이라 완충 자동, 손복제본
freeway_substep_local 한 곳에 동결 buffer_bc 스레딩(링크 follower+seg13 frozen 둘 다 커버).
**최종 재실행(768ac85)**: 원래 5셀 × {Wu, PFO(box), P-CENT, walk-MVG 원래 플래그} 20런 진행 중.
가정-경계 시대 그리드(_fullsweep/paper3)는 대조 데이터로 보존.
