...
## SPLIT-PRICE 사가 (v0→v1→v2) — 플래그십 최종 확정

발견: 사용자가 승인했다고 인지한 설계(총량 equality + 배분에 가격)와 구현(B3 계보:
가격이 레벨 조절)이 달랐음. v1(split 구현) → hybrid가 전 depth에서 level에 패배 →
로그 분석으로 병인 확정: incumbent(가격-자율 레벨)와 leader 후보(budget)의 **레짐
플래핑**(Σmeter TV 1.74×, 과소방류 −800, 신호B green −12s). v2(incumbent 가격-레벨
배제) → 3지표 동시 회복 + TTT 12566→11893(−673). 인과 사슬 완결.

### 3구조 depth 지도 (d3 sweet_190, LINK_SHARE=off)
| depth | G1DF(수량) | level/v0(가격레벨) | hybrid-v2(수량+배분가격) |
|---|---:|---:|---:|
| d0 | 11847 | **22698 붕괴** | 13079 무붕괴(v1) |
| d1 | 12050 | 11814 (재현 11813.918 완전일치) | - |
| d2 | 12277 | 12006 | - |
| d3 | **11283** | 12043 | **11893** |
| d4 | **11230** | **11685** | (실행중) |
| d5 | 11602 | 11789 | - |
| d6 | 11334 | 11773 | - |

- level: d0 붕괴 외엔 **11.7~12.0k 고원**(깊이 둔감, 비용만 증가 58→97s) — "d1 스윗스팟"
  가설은 노이즈 과적합이었음(±300 이내 depth 서열은 단일런으로 주장 금지).
- hybrid-v2: G1DF형 곡선 + d0 무붕괴 + level 최적과 동급(11893 vs 11685~11814).
