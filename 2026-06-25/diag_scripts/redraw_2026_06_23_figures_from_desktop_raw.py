...
sys.path.insert(1, str(CURRENT_ROOT))
import matplotlib
matplotlib.use("Agg")
...
from src.models.demand import DemandProfile, ScenarioConfig, load_scenarios, apply_scenario_network_overrides
...
D1 = str(DESKTOP_ROOT / "outputs" / "analysis_matrix_3600")
D2 = str(DESKTOP_ROOT / "outputs" / "analysis_matrix_3600_extra")
OUT = str(CURRENT_ROOT / "reports" / "figures" / "redrawn_2026_06_23_from_desktop_raw"); os.makedirs(OUT, exist_ok=True)
...
scen.update({
    "medium_demand": scen.get(
        "medium_demand",
        ScenarioConfig("medium_demand", urban_scale=1.0, freeway_scale=1.0, ramp_scale=1.0),
    ),
    "peak_demand": scen.get(
        "peak_demand",
        ScenarioConfig("peak_demand", urban_scale=1.25, freeway_scale=1.25, ramp_scale=1.25),
    ),
    "heavy_demand_140": ScenarioConfig(
        "heavy_demand_140", urban_scale=1.40, freeway_scale=1.40, ramp_scale=1.40
    ),
    "heavy_demand_150": ScenarioConfig(
        "heavy_demand_150", urban_scale=1.50, freeway_scale=1.50, ramp_scale=1.50
    ),
    "oversaturated_demand": ScenarioConfig(
    ...
    "incident_or_capacity_drop": ScenarioConfig(
    ...
    "skew_peak": ScenarioConfig(
        "skew_peak",
        urban_scale=1.25,
        freeway_scale=1.25,
        ramp_scale=1.25,
        urban_boundary_weight_override={
            "in_C_top": 1.8,
            "in_C_right": 1.8,
            "in_F_right": 1.8,
        },
    ),
    "skew_heavy": ScenarioConfig(
        "skew_heavy",
        urban_scale=1.40,
        freeway_scale=1.40,
        ramp_scale=1.40,
