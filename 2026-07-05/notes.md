...
## 13. (2026-07-06) 사용자 질문 — "receding horizon인데 왜 절벽을 못 보나"의 실증 분해

사용자 반론: 9분 창을 3분마다 미는데 "수십 분 뒤라 못 본다"는 틀린 설명 아닌가 — **맞는
지적**. 창은 절벽을 결국 본다. B3CERT 런의 가격·밀도·metering 시계열 직독으로 정정:

| 국면 | step | ρ_E(임계 33.5) | 가격(R_D_E) | 미분의 상태 |
|---|---|---|---|---|
| 절벽 전 | 0–2 | 25→31 | **+0.01~+0.05 (조여라, 옳음)** | 있으나 약함 |
| 절벽 통과 | 3–6 | 39→53 | +0.003 → **−0.447 (풀어라?!)** | 부호 붕괴(쓰레기) |
| 절벽 후 | 7–39 | 88→92 jam | **정확히 0.0000 영구** | 소멸 |

- **정정된 명제**: "창이 못 본다"가 아니라 **"창이 보는 순간 미분이 무의미해진다"**.
  (1) 절벽 전: 유한차분은 9분·±δ의 효과만 재는데 진짜 반사실은 30분 지속 감축 — 누적
  인과가 1차 항에 과소 반영(boiling frog). (2) 경계: 불연속을 가로지르는 secant는 hi/lo
  분기의 붕괴 타이밍이 어긋나며 크고 부호 불안정한 값(결정적 순간에 −0.447로 역방향).
  (3) jam 후: merge가 공급 제한이라 metering이 non-binding — ±δ가 아무것도 못 바꿔
  차분 정확히 0(회복은 δ·horizon 모두 초과하는 조치라 국소 gradient에 불가시).
- cert는 전 구간 인증 0으로 제 역할(경계 국면의 역방향 가격이 상향을 밀 때 차단).
- 제약(ceiling)의 우월성 재확인: 세 국면 어디에도 의존하지 않고 원인 수준을 직접 제한.
- §12 정리의 실증적 기초 완성: 절벽 레버에서 1차 가격은 (약함→쓰레기→0)의 궤적을
  그리므로, 그 위에 어떤 안전장치를 얹어도 가격 성분의 한계 기여는 0으로 수렴한다.

## 14. (2026-07-06) 사용자 질문 2제 — leader 페널티는 왜 못 막았나 + F1 아키텍처 합의

**Q: leader objective에 페널티가 있는데 왜 범람을 못 막나? 애초에 거기서 막았어야 하는
것 아닌가?** — A(데이터): **막고 있었다, 등식 budget이 연결돼 있을 때는.** breakdown
진입기(step 4-10) 비교:

| | leader N_UF | follower 실제 metering(R_D_E) |
|---|---|---|
| B2TR(등식) | 4500~4800 적극 하향 | **750~1050, budget 추종** → 무붕괴 |
| B3CERT(가격+soft) | 4650~5550 비슷 | **1500 고정, budget 무시** → 붕괴 |

leader 페널티는 후보 평가로만 작동한다 — 등식에선 N_UF 후보=metering이라 페널티가 곧
조향. 가격 모드는 budget을 soft(w=T_c_h, 미약)로 풀어 **어떤 N_UF 후보든 follower 응답이
동일**해졌고, 후보 간 차등이 사라져 페널티가 무력화됐다. **원리: 안전 페널티는 결정이
실제로 내려지는 곳에 붙어야 작동한다(권한을 위임하면 안전도 함께 이사).**

**구현 정정(정직 공개)**: B3CERT의 증명서는 사실상 무력했다 — metering 탐색이 매 solve
snapshot 규약상 capacity에서 재시작하는데, cert는 "직전 commit 대비 상향 이동"만 거부
하므로 시작점이 이미 꼭대기라 거부할 이동이 없었다. 고치면 과소방류 나선으로 갈 뿐
(결론 불변)이나 §11-12 해석에 이 뉘앙스를 추가한다.

**F1 아키텍처 합의(사용자 제안)**: leader = 순수 TTT 가격(green/offset/metering/VSL) +
N_P/N_UF; follower objective에 안전 페널티 이관 — urban: own-TTS + 0.5cap hinge,
freeway: own-TTS + ρ_crit hinge. 근거: (i) 절벽 정보가 미분을 통과하지 않고 follower의
**후보 단위 비선형 평가**(국소 정책 비교)로 전달됨 — §13의 3국면 병리가 발생할 자리가
없음. (ii) 자율 PFO가 190에서 무붕괴인 것이 증거(국소 METANET이 자기 절벽을 이미 봄;
페널티는 이를 명시적 마진으로 승격 + leader budget에 대한 안전 거부권 부여). (iii)
B2BAR와의 차이: 페널티가 후보 간 상수면 argmin 중립 → 190 회귀 위험이 구조적으로 낮음.
(iv) 위 원리의 구현: 안전이 권한(각 레버의 실제 결정처)을 따라간다. 검증 순서:
F1(페널티 이관, 무회귀 확인) → F2(metering 가격+trust 재도전 — 절벽은 follower가
지키므로 가격은 매끈한 몫만) → F3(offset 가격 — legacy 잔존 격차 재료).
