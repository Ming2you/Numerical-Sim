...
while :; do
  ...
  for c in 155 170 170skew 170inc 190; do
    f="outputs/_diag/S_${c}/$PS/run_log.csv"
    n=$( [ -f "$f" ] && wc -l < "$f" || echo 0 ); [ "$n" -lt 80 ] && done=0
  done
  [ "$done" = 1 ] && break; sleep 30
done
echo "===== R41 SMOOTH DONE ====="
...
print('='*88)
print(f'{\"cell\":8}{\"PFO\":>10}{\"이산3분기\":>11}{\"연속\":>10}{\"gap\":>10}')
w=0;n=0
for cell,nm in [('155','Low'),('170','Med'),('170skew','Skew'),('170inc','Inc'),('190','High')]:
    pfo=W(load(f'pfosplit_{cell}',FF)); disc=W(load(f'FIN2_{cell}')); a=W(load(f'S_{cell}'))
