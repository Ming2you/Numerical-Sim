...
while :; do
  ...
  for d in $(ls -d outputs/_diag/G_*_190 2>/dev/null); do
    n=$( [ -f "$d/$PS/run_log.csv" ] && wc -l < "$d/$PS/run_log.csv" || echo 0 ); [ "$n" -lt 80 ] && done=0
  done
  cnt=$(ls -d outputs/_diag/G_*_190 2>/dev/null | wc -l)
  [ "$cnt" -lt 12 ] && done=0
...
rows=[(W(load(f'outputs/_diag/F_f050wf020_190/{PS}/run_log.csv'))-pfo,'R34최고(f0.5,wf0.20)')]
for d in sorted(glob.glob('outputs/_diag/G_*_190')):
    t=os.path.basename(d)[2:-4]
    v=W(load(f'{d}/{PS}/run_log.csv'))
    if v is not None: rows.append((v-pfo,t))
print(f'R35 2D 격자  PFO={pfo:.2f}')
for gap,t in sorted(rows):
    print(f'  {t:20}: vsPFO {gap:+8.2f}{\"  ★WIN\" if gap<0 else \"\"}')
b=min(rows); print(f'\n  최고: {b[1]} gap={b[0]:+.2f}' + ('  → ★★High 돌파! 5/5 가능' if b[0]<0 else '  → 계속'))
"
