#!/bin/bash
set -u
CELL=$1; THR=${2:-10}; HOLD=${3:-3}; W=${4:-0.013}
...
export WARMUP_NC_STEPS=5 FW_BUFFER=8 TERM_ZG=1 VFREE=115 RHO_CRIT=31.5 TAU_H=0.0056111 \
       ...
       MS_ADAPT=1 MS_THR="$THR" MS_HOLD="$HOLD" MS_W="$W"
OUT="outputs/_adapt/t${THR}h${HOLD}w${W}_${CELL}"; mkdir -p "$OUT"
...
echo "[$(date +%H:%M)] done adapt ${CELL} thr=$THR hold=$HOLD w=$W rc=$? rows=$(wc -l < "$OUT/P-STACK-WU-FAITHFUL-ALLPRICE-JOINT/run_log.csv" 2>/dev/null || echo 0)"
