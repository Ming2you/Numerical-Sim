#!/usr/bin/env bash
# 공정 지평 H=4(720s) — H3와 H6 사이 해상도 보강 (2026-07-25, 사용자 요청)
# HORIZON=4를 PFO·P-Stack 동일 적용(공정). leader_value_depth=0 불변.
set -u
...
for cell in 170inc 190 170; do
  out="outputs/_diag/fh_ps_H${H}_${cell}"; mkdir -p "$out"
  env -u SEG13 -u METER_BOX -u VSL_BOX -u SUP_PFO -u SUP_GATE "${BASE_ENV[@]}" "${SPILL_ENV[@]}" HORIZON="$H" \
    "$PY" -u work/run_claude_style_five_controller.py \
      ...
      > "$out.log" 2>&1 &
  out2="outputs/_diag/fh_pfo_H${H}_${cell}"; mkdir -p "$out2"
  env -u SEG13 -u METER_BOX -u VSL_BOX -u SUP_PFO -u SUP_GATE "${BASE_ENV[@]}" HORIZON="$H" \
    "$PY" -u work/run_claude_style_five_controller.py \
      --scenario "${SC[$cell]}" --T-total 14400 --controllers "$FF" --output "$out2" \
      > "$out2.log" 2>&1 &
done
wait
echo "===== FAIR-HORIZON H4 DONE ====="
for cell in 170inc 190 170; do
  echo "  ps H4 ${cell} rows=$(wc -l < "outputs/_diag/fh_ps_H4_${cell}/$PS/run_log.csv" 2>/dev/null)"
  echo "  pfo H4 ${cell} rows=$(wc -l < "outputs/_diag/fh_pfo_H4_${cell}/$FF/run_log.csv" 2>/dev/null)"
done
