...
while :; do
  ...
  for c in 155 170 170skew 170inc 190; do
    f="outputs/_diag/sup2_${c}/$PS/run_log.csv"
...
def W(d,col='cumulative_total_ttt'):
    ...
    return c[end]-c[bi]
print('R10 spillback+SUP_PFO 전 5셀 — 성공기준 PFO 전부 이김')
...
print('감독자 픽 진단(sup_pick_pfo=1 비율, PFO 위임 스텝수/80):')
for cell,nm in [('155','Low'),('170','Med'),('170skew','Skew'),('170inc','Inc'),('190','High')]:
    p=f'outputs/_diag/sup2_{cell}/{PS}/run_log.csv'
    if not os.path.exists(p): continue
    d=pd.read_csv(p)
    if 'sup_pick_pfo' not in d.columns: print(f'  {nm}: 컬럼없음'); continue
    v=pd.to_numeric(d['sup_pick_pfo'],errors='coerce').fillna(0)
    print(f'  {nm:6}: PFO위임 {int(v.sum())}/{len(v)} 스텝 ({100*v.mean():.0f}%)')
"
