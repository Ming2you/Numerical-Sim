#!/usr/bin/env bash
# R37 — High 승리 구성이 5셀 전체에 통하는가 (단일 설정 통합 시도)
# High 승리: METER/VSL/OFFSET price off + BUDGET_FAIR + spillback(WU2,WF0.20,nref800,lead0.5)
#            + MFD_FAR_W_FREEWAY=0.5, 감독자X, 예산O  → -1.20 WIN
set -u
