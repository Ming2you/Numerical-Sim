#!/bin/bash
# METER_SMOOTH 단독 A/B (NUF_PROJ 없음) — 기준선 = outputs/_nufproj/off_*
set -u
W=$1; CELL=$2
...
export WARMUP_NC_STEPS=5 FW_BUFFER=8 TERM_ZG=1 VFREE=115 RHO_CRIT=31.5 TAU_H=0.0056111 \
       ...
       BIAS_SAMPLE=1 BIAS_POW=0.4 NASH_SMAX=10 METER_SMOOTH="$W"
[ -n "${VS:-}" ] && export VSL_SMOOTH="$VS"
OUT="outputs/_ms/w${W}${VS:+_v$VS}_${CELL}"; mkdir -p "$OUT"
...
echo "[$(date +%H:%M)] done w=$W ${CELL} rc=$? rows=$(wc -l < "$OUT/P-STACK-WU-FAITHFUL-ALLPRICE-JOINT/run_log.csv" 2>/dev/null || echo 0)"
