#!/usr/bin/env bash
# R18 — 통합 단일 규칙 AUTH_ADAPT 검증 (2026-07-26)
# 하나의 컨트롤러 설정으로 5셀 전부. 셀별 튜닝 아님 — (예보 수요대역 × 교란감지) 상태 규칙.
# 기대(셀별 최고 재현): Low -6 / Med -95 / Skew -26 / Inc -47 / High(별도 WF 확정 후)
set -u
...
for cell in "$@"; do
  out="outputs/_diag/auth3_${cell}"; mkdir -p "$out"
