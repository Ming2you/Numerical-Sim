...
export WARMUP_NC_STEPS=5 FW_BUFFER=8 TERM_ZG=1 VFREE=115 RHO_CRIT=31.5 TAU_H=0.0056111 \
       ...
       BIAS_SAMPLE=1 BIAS_POW=0.4 NASH_SMAX=10 BUDGET_OFF=1
OUT="outputs/_budgetoff/${CELL}"; mkdir -p "$OUT"
...
echo "[$(date +%H:%M)] done budgetoff ${CELL} rc=$? rows=$(wc -l < "$OUT/P-STACK-WU-FAITHFUL-ALLPRICE-JOINT/run_log.csv" 2>/dev/null || echo 0)"
