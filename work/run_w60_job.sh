...
case "$TAG" in
  ...
  pstack|pstackoff)
    ...
    if [ "$TAG" = "pstack" ] && [ "$CELL" = "170skew" ]; then
      export LEADER_GRID_SWEEP="outputs/_w60/${TAG}_${CELL}/grid" LEADER_GRID_STEP=17 GRID_N=9 \
             RESIDUAL_LOG="outputs/_w60/${TAG}_${CELL}/resid" LEADER_CAND_LOG="outputs/_w60/${TAG}_${CELL}/cand"
    fi ;;
  *) echo "unknown tag $TAG"; exit 1 ;;
esac

OUT="outputs/_w60/${TAG}_${CELL}"
mkdir -p "$OUT"
