# 밤샘 full sweep 롤링 큐(2026-07-19) — fullsweep_jobs.txt를 최대 동시 N개로 순차 발주
param([int]$MaxConc = 10)
Set-Location "C:\Users\alsrj\Desktop\Numerical-Sim-offiter"
$PY = "C:\Users\alsrj\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe"
$jobs = Get-Content "work\fullsweep_jobs.txt" | Where-Object { $_ -and -not $_.StartsWith("#") }
...
foreach ($job in $jobs) {
  ...
  Add-Content "outputs\_logs\fullsweep_queue.log" "[$(Get-Date -Format 'HH:mm:ss')] dispatch $i/$total $tag"
  Start-Sleep -Seconds 2
}
"QUEUE DISPATCH COMPLETE $(Get-Date)" | Out-File "outputs\_logs\fullsweep_dispatch_done.txt"
