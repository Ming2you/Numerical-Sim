...
OUT=outputs/_diag/anchor_smoke_170inc; mkdir -p "$OUT"
...
echo "ANCHOR SMOKE rc=$? rows=$(wc -l < "$OUT/P-STACK-WU-FAITHFUL-ALLPRICE-JOINT/run_log.csv" 2>/dev/null) err=$(grep -icE 'error|traceback' "$OUT.log")"
