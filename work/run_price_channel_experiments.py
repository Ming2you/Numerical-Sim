...
# 2026-07-04 병합 포팅: 가격 API가 signal/metering/vsl_marginal_price(동결 운영점 g_ext,
# hold+event-trigger refresh)로 통일됨 — 변형 의미는 Codex 원안 유지, B4(barrier) 추가.
VARIANTS = [
    "NO-CONTROL",
    "PFO-PURE",
    "PFO-TAX-B3",
...
class PriceOnlyFollower:
    ...
    def __init__(self, cfg: ExperimentConfig):
        self.cfg = cfg
        # helper의 refresh 기계(hold+event-trigger, 동결 운영점 g_ext)를 그대로 빌려
        # 가격을 계산하고, leader-search 없는 standalone follower에 복사해 쓴다.
        self.helper = StackelbergWuMeteredController(cfg)
        self.helper.signal_price_enabled = True
    ...
    def decide(self, state, forecast, previous: Optional[ControlAction]) -> ControlAction:
        ...
        self.helper._maybe_refresh_signal_prices(state.copy(), list(forecast), prev)
        src = self.helper.nash_solver
        self.follower.signal_marginal_price = src.signal_marginal_price
        self.follower.signal_marginal_price_ref = dict(src.signal_marginal_price_ref)
        self.follower.metering_marginal_price = src.metering_marginal_price
        self.follower.metering_marginal_price_ref = dict(src.metering_marginal_price_ref)
        self.follower.vsl_marginal_price = src.vsl_marginal_price
        self.follower.vsl_marginal_price_ref = dict(src.vsl_marginal_price_ref)
        try:
            return self.follower.solve(state.copy(), None, forecast, prev).control
        finally:
            self.follower.signal_marginal_price = None
            self.follower.metering_marginal_price = None
            self.follower.vsl_marginal_price = None
...
def make_controller(variant: str, cfg: ExperimentConfig):
    ...
    if variant in {"PSTACK-B2-OFF", "PSTACK-B2-ON", "PSTACK-B3", "PSTACK-B4"}:
        controller = StackelbergWuMeteredController(cfg)
        controller.signal_price_enabled = variant in {"PSTACK-B2-ON", "PSTACK-B3", "PSTACK-B4"}
        controller.metering_price_enabled = variant in {"PSTACK-B3", "PSTACK-B4"}
        controller.vsl_price_enabled = variant == "PSTACK-B3"
        controller.barrier_price_enabled = variant == "PSTACK-B4"
        if variant in {"PSTACK-B3", "PSTACK-B4"}:
...
def run_one(variant: str, scenario_name: str, t_total: float, output_root: Path, reference) -> Dict[str, Any]:
    ...
    for step in range(steps):
        ...
        decision_rows.append({
            ...
            "wu_b2_price_enabled": float(diag.get("wu_b2_price_enabled", 0.0)),
            "wu_b2_price_refreshed": float(diag.get("wu_b2_price_refreshed", 0.0)),
            "wu_b3_meter_price_enabled": float(diag.get("wu_b3_meter_price_enabled", 0.0)),
            "wu_b3_vsl_price_enabled": float(diag.get("wu_b3_vsl_price_enabled", 0.0)),
            "wu_b4_barrier_enabled": float(diag.get("wu_b4_barrier_enabled", 0.0)),
            "leader_pfo_incumbent_selected": float(diag.get("leader_pfo_incumbent_selected", 0.0)),
        })
        # 가격 진단(wu_b2_/wu_b3_)은 control_row에 안 실리므로 여기서 수집.
        decision_rows[-1].update({
            k: float(v) for k, v in diag.items()
            if k.startswith(("wu_b2_price_", "wu_b3_meter_price_")) and isinstance(v, (int, float))
        })
