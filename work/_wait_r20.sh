...
while :; do
  ...
  for c in 155 170 170skew 170inc 190; do
    f="outputs/_diag/auth3_${c}/$PS/run_log.csv"
    n=$( [ -f "$f" ] && wc -l < "$f" || echo 0 ); [ "$n" -lt 80 ] && done=0
  done
  [ "$done" = 1 ] && break; sleep 30
done
echo "===== R19 AUTH_ADAPT v3(래치) ALL DONE ====="
...
def W(d,col='cumulative_total_ttt'):
    ...
    return c[end]-c[bi]
print('='*84)
print('AUTH_ADAPT v3(래치) — 단일 컨트롤러 규칙, 5셀 (음수 = PFO 이김)')
print('='*84)
...
for cell,nm in [('155','Low'),('170','Med'),('170skew','Skew'),('170inc','Inc'),('190','High')]:
    ...
    a=W(load(f'auth3_{cell}',PS)); pc=W(load(f'pcent_{cell}','P-CENT'))
