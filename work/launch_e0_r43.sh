#!/usr/bin/env bash
# R42 — ★진짜 3스텝 리더: LEADER_V_DEPTH=0 (+FAR_D0=1로 터미널 항 유지)
# 정정: ALLPRICE-JOINT는 LEADER_V_DEPTH 미지정 시 자동으로 3을 넣어 리더 rollout이 3+3=6이 된다.
#   "안 건드림" = depth 3. 3스텝으로 만들려면 명시적으로 0을 줘야 함.
#   depth=0이면 far/spillback 가산 게이트(depth>0)가 닫히므로 FAR_D0=1로 같은 채점 형태 유지.
set -u
...
BASE_ENV=(WARMUP_NC_STEPS=5 FW_BUFFER=8 TERM_ZG=1 VFREE=115 RHO_CRIT=31.5 TAU_H=0.0056111
  ...
  AUTH_ADAPT=1 AUTH_SMOOTH=1 AUTH_TRUST_BIG=6.0 AUTH_TRUST_SMALL=1.5)
for cell in "$@"; do
  out="outputs/_diag/E0_${cell}"; mkdir -p "$out"
