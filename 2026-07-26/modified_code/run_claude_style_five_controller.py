...
def run_one(controller_id: str, scenario_name: str, t_total: float, output_root: Path, reference) -> Dict[str, Any]:
    ...
    for step in range(start_step, steps):
        ...
        if _os.environ.get("AUTH_ADAPT") == "1":
            ...
            _trust = _t_big if (_dem >= _d_lo and not _disrupt_eff) else _t_small
            # 고부하 국면엔 green trust도 승리구성(기본 6) 유지 — 저수요 축소 규칙이 배수기를 깨지 않게.
            if _high:
                _trust = _t_big
            globals()["_AUTH_HIGH"] = bool(_high)
            if hasattr(controller, "signal_price_trust_sec"):
                controller.signal_price_trust_sec = _trust
            # 고수요: freeway 가격 채널 해제 + spillback freeway 가중 부여
            if hasattr(controller, "metering_price_enabled"):
                controller.metering_price_enabled = not _high
            if hasattr(controller, "vsl_price_enabled"):
                controller.vsl_price_enabled = not _high
            cfg.mpc.leader_spillback_freeway_weight = _hi_wf if _high else 0.0
            _auth_sig = (round(_trust, 3), bool(_high), bool(_disrupt_eff))
            if _auth_sig != globals().get("_AUTH_LAST"):
                ...
                print(
                    ...
                    f"high={int(_high)} trust={_trust} fw_w={cfg.mpc.leader_spillback_freeway_weight}",
                    flush=True,
                )
