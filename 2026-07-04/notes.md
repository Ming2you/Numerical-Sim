...
## 10. (2026-07-05) barrier 개정판(선형 hinge·veh·h + urban 항) — **전면 음성, barrier 계획 마감**

사용자 지적 2건(단위: 차량수 환산·선형화 / urban critical capacity 0.5·cap 초과 penalty)을
반영한 개정(9683d4d): barrier = w·Σ[max(0,ρ−ρ_c)·L·lanes + max(0, frac·cap − S_eff)]·T_c_h,
단위 veh·h(TTT와 동일, w=1이 1차 정확값), green·metering 두 가격 채널 모두에 합산.

**probe(혼잡 상태, PFO 20-step 전진 생성)**: 선형화로 freeway 항이 metering 축에서 부활
(+0.001~0.010, 어제 제곱의 ~75배). urban 항은 metering 축에서 0.2-frac 사멸/0.5-frac 약함
(|g_TTT|의 1/5~1/10, 부호도 혼재 — 과잉조임 견제 몫은 이미 P1·전역 TTT가 지고 있음),
**green 축에서 강함**(0.2: A +0.055 / 0.5: C −0.210, A/B/F +0.03~0.04).

**closed-loop 7200s 판정**:

| | sweet_155 | sweet_190 |
|---|---:|---:|
| 가격 OFF | **4419.7** | 12846.6 |
| B2(barrier 없음) | 4873.7(+10.3%) | **12409.5(−3.4%)** |
| B2BAR(green+barrier, frac 0.2) | 4671.6(+5.7%) | 13240.2(+3.1%) |
| B2BAR50(frac 0.5, 사용자 기준) | **5057.8(+14.4%)** | (155 탈락으로 미실행) |

- **frac 0.2 = regime 맞교환**: 155 폭발의 45% 회수 ↔ 190 이득 전멸·기준선 이하. 기전:
  190 과포화에선 urban 링크가 **상시** 만차 근처(§ P1.5 계측 x 2.6~4.3, spillback base
  62 veh·h)라 "조여질 때만 발화" 전제가 깨지고 barrier가 상시 발화하며 B2 이득의 원천인
  TTT 조정 가격을 익사시킴.
- **frac 0.5 = 순수 악화**: 조기·광역 발화가 중부하 왜곡을 증폭. probe의 유망 gradient
  (C −0.21)는 닫힌 루프에서 역작용 — 정적 상태항의 open-loop gradient는 closed-loop
  효과를 예측하지 못한다(B1→B2 전이는 성공했지만 barrier에선 실패한 지점).
- **처분**: barrier 코드는 보존(전 변형 기본 OFF·opt-in), barrier로 B2.1을 푸는 계획은 마감.
- **B2.1 윤곽 갱신 — 세 번째 독립 실패**(P1.5 포화도 게이트 → 가격 크기 → barrier):
  155↔190을 가르는 것은 **상태의 정적 함수가 아니다**. 유력 가설: 중부하에선 운영점이
  진동(큐 생성↔소멸)하는데 동결 선형 가격이 지연 추적하며 진동을 증폭(limit cycle);
  190 과포화에선 운영점이 정착해 가격이 안정 작동. 고전 처방 = **감쇠**: (a) w<1 스윕
  (0.25/0.5, sweet_155 7200s — 155 폭발이 잦아들며 190 이득 일부 보존되면 가설 확증),
  (b) 가격의 EMA/이동평균, (c) refresh 시 재선형화 지연 보상. 다음 세션 1순위.
