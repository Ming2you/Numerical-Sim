...
def make_controller(controller_id: str, cfg: ExperimentConfig):
    ...
    if controller_id == "P-STACK-WU-FAITHFUL-ALLPRICE-JOINT":
        # DEFAULT = SPLIT-PRICE v2. 구 level 모드(가격-레벨)는 METER_PRICE_MODE=level.
        # ★2026-07-27 제거(사용자 지시): 여기서 LEADER_V_DEPTH 미지정 시 leader_value_depth를
        # 자동으로 3으로 올리는 숨은 기본값이 있었다. 그래서 "depth를 건드리지 않는다"가
        # 곧 리더 rollout = horizon(3) + depth(3) = 6스텝을 의미했고, PFO(3스텝)와의 비교가
        # 동일 지평이 아니었다. 이제 기본은 state.py 정의값(leader_value_depth=0) = 리더도 3스텝.
        # 깊이가 필요하면 env LEADER_V_DEPTH로 **명시적으로만** 준다.
        # 주의: depth=0이면 far/spillback 가산 게이트(`depth>0 or far_at_d0`)가 닫히므로,
        # 터미널 항을 쓰려면 FAR_D0=1을 함께 지정해야 한다.
        import os as _os_ap
        ...
        # 채널별 가격 토글(2026-07-26): 기존엔 ALLPRICE_OFF(일괄)뿐이라 "어느 채널이 손해인가"를
        # 분리 못 했다. 진단(2026-07-25): Inc는 freeway 좋고 urban 나쁨(=metering 과잉 서명),
        # High는 urban 나쁨. 채널 gate는 잔존가격+cert까지 지우므로(stackelberg_wu_metered L1094~)
        # weight만 0으로 두는 반쪽 해제 함정은 없다. 미지정=기존 거동.
        if _os_ap.environ.get("METER_PRICE") == "0":
        ...
        if _os_ap.environ.get("VSL_PRICE") == "0":
            controller.vsl_price_enabled = False
        if _os_ap.environ.get("METER_PRICE_W") and hasattr(
            getattr(controller, "nash_solver", None), "metering_marginal_price_weight"
        ):
            # metering 한계가격 권한 세기(1.0=기본). Inc/High가 서로 다른 세기를 원하는지 sweep용.
            controller.nash_solver.metering_marginal_price_weight = float(
                _os_ap.environ["METER_PRICE_W"]
            )
...
def run_one(controller_id: str, scenario_name: str, t_total: float, output_root: Path, reference) -> Dict[str, Any]:
    ...
    # PREFILTER_TOPK(2026-07-26): grid 경로 리더는 후보 ~48개 중 싼 proxy로 고른 **4개만**
    # full 평가한다(stackelberg_prefilter_top_k=4 기본, 실측 leader_candidate_count 48 vs
    # prefilter_selected 4.2). 예산이 "잘 배분"되지 못하는 병목 후보 — 열어서 A/B.
    # BUDGET_FAIR가 무력했던 이유도 이것일 수 있다(격자를 고르게 뽑아도 prefilter가 다시 4개로).
    if _os.environ.get("PREFILTER_TOPK"):
        cfg.mpc.stackelberg_prefilter_top_k = int(_os.environ["PREFILTER_TOPK"])
    if _os.environ.get("PREFILTER_LOCAL_TOPK"):
        cfg.mpc.stackelberg_prefilter_local_top_k = int(_os.environ["PREFILTER_LOCAL_TOPK"])
    ...
    if _os.environ.get("URBAN_CLF") == "1":
        # Mod1(2026-07-25 자율재설계): urban terminal cost를 operating-point 기준 볼록 CLF +
        # 기울기 외삽으로 재설계. flat objective·가격붕괴·과-metering 근본 치료.
        cfg.mpc.leader_urban_clf_enabled = True
    if _os.environ.get("URBAN_CLF_GATED") == "0":
        cfg.mpc.leader_urban_clf_gated = False  # always-on(全셀 CLF). 기본 gated(gate셀만).
    if _os.environ.get("SPILLBACK") == "1":
        # 대칭 성장-투영 spillback(2026-07-25): urban(경계포함)·freeway 성장 선반영, 균형 유도.
        cfg.mpc.leader_spillback_enabled = True
    for _e, _f in (("SPILLBACK_W", "leader_spillback_weight"),
                   ("SPILLBACK_WU", "leader_spillback_urban_weight"),
                   ("SPILLBACK_WF", "leader_spillback_freeway_weight"),
                   ("SPILLBACK_LEAD", "leader_spillback_lead_h"),
                   ("SPILLBACK_NREF_U", "leader_spillback_nref_urban"),
                   ("SPILLBACK_NREF_F", "leader_spillback_nref_freeway")):
        if _os.environ.get(_e):
            setattr(cfg.mpc, _f, float(_os.environ[_e]))
    if _os.environ.get("MOVE") == "1":
        # control-move penalty(2026-07-25): budget 급변 벌점 → 매끈(whipsaw 억제, 오라클 모사).
        cfg.mpc.leader_move_enabled = True
    for _e, _f in (("MOVE_W", "leader_move_weight"),
                   ("MOVE_WNUF", "leader_move_nuf_weight"),
                   ("MOVE_WNP", "leader_move_np_weight")):
        if _os.environ.get(_e):
            setattr(cfg.mpc, _f, float(_os.environ[_e]))
    for _e, _f in (("URBAN_CLF_NREF", "leader_urban_clf_nref"),
                   ("URBAN_CLF_NCRIT", "leader_urban_clf_ncrit"),
                   ("URBAN_CLF_NJAM", "leader_urban_clf_njam"),
                   ("URBAN_CLF_PMAX", "leader_urban_clf_pmax"),
                   ("URBAN_CLF_GMIN", "leader_urban_clf_gmin"),
                   ("URBAN_CLF_TTAIL", "leader_urban_clf_ttail")):
        if _os.environ.get(_e):
            setattr(cfg.mpc, _f, float(_os.environ[_e]))
    ...
    def _sup_score(ctrl: ControlAction, forecast) -> float:
        """감독자 공통 채점: 고정 control로 h3 결합 rollout TTT + far + spillback(리더 V와 동형).

        ★spillback 추가(2026-07-25): 기존 h3(rollout+far)은 근시라 Med/Skew서 오판→whipsaw
        (camp_s10off Med 4106=P-Stack·PFO 둘다↓). 리더 실제 목적이 rollout+far+spillback이므로
        감독자도 동일 항으로 채점해야 P-Stack≥PFO 보장이 실효. spillback은 states 목록 필요."""
        from src.simulation.coupling import run_coupled_interval
        from src.controllers.stackelberg_mpc import mfd_far_cost_to_go, leader_spillback_cost
        s = sim.state.copy()
        states = [s.copy()]
        total = 0.0
        ...
        total += float(leader_spillback_cost(cfg, states))
        return total
    ...
    for step in range(start_step, steps):
        ...
        # ---- GREEN_TRUST_ADAPT(2026-07-26): 수요 기반 green authority 자동 조절 ----
        # 실측(R14): Incident/저부하는 작은 green trust(1.5)가 +328→+44로 최선인데, High는 같은
        # 값이 +292로 악화하고 큰 authority를 원한다. 단일 고정값으로 둘을 동시에 만족 못 함.
        # 시나리오명이 아니라 **예보 총수요**로 분기 — 일반 규칙이고 지평도 안 늘리므로 공정.
        # 실측 분리: Low 21778 / Med·Skew·Inc 23886 / High 26696 → 임계 25000이면 High만 분리.
        # AUTH_ADAPT(2026-07-26): 상태(예보 수요대역 × 교란감지) 기반 **권한 자동 조절** 단일 규칙.
        # 시나리오명 하드코딩 아님. 깊이(horizon) 불변이라 PFO/P-CENT 공정비교 유지.
        # 실측 근거(R14~R16, vs PFO):
        #   Low(수요21778)      : green trust 1.5   → -6
        #   Med/Skew(23886,정상): green trust 6      → -95 / -26
        #   Inc(23886,교란)     : green trust 1.5   → -47   ← 수요 동일, 교란으로 분리
        #   High(26696)         : meter+VSL 가격 off + spillback freeway가중 0.25 → +2
        # 물리적 해석: (i) 교란 중엔 green 한계가격 추정이 불신뢰 → 권한 축소,
        #   (ii) 저수요는 green 이동의 한계가치가 낮음 → 권한 축소,
        #   (iii) 고수요에선 freeway 가격이 역효과(과보호) → 해제하고 예산으로만 조정.
        if _os.environ.get("AUTH_ADAPT") == "1":
            _d_lo = float(_os.environ.get("AUTH_DEM_LOW", "22500"))
            _d_hi = float(_os.environ.get("AUTH_DEM_HIGH", "25000"))
            _t_small = float(_os.environ.get("AUTH_TRUST_SMALL", "1.5"))
            _t_big = float(_os.environ.get("AUTH_TRUST_BIG", "6.0"))
            _hi_wf = float(_os.environ.get("AUTH_HIGH_WF", "0.25"))
            _fs = forecast[0] if forecast else None
            _dem = 0.0
            if _fs is not None:
                _dem = (
                    sum(float(v) for v in getattr(_fs, "freeway_mainline", {}).values())
                    + sum(float(v) for v in getattr(_fs, "urban_boundary", {}).values())
                    + sum(float(v) for v in getattr(_fs, "ramp_arrival", {}).values())
                )
            # 교란 감지 = 예보 지평 내 차로폐쇄(FAR_GATE mode1과 동일 신호, 지연 없음).
            from src.models.demand import merge_freeway_lane_loss as _mfl
            _disrupt = any(
                float(v) > 0.0
                for _segs in _mfl(list(forecast)).values()
                for v in _segs.values()
            )
            # 교란 래치(2026-07-26): 예보 폐쇄가 끝나도 망은 한동안 교란 상태다(캐스케이드 정점이
            # 결정보다 ~17스텝 뒤). 래치 없이 풀면 회복 임계구간(Inc step35~41)에 authority가
            # 튀어 +685로 파국(실측). FAR_GATE mode1과 동일하게 **밀도 회복 완료 시에만 해제**.
            if _disrupt:
                globals()["_AUTH_LATCH"] = True
            elif globals().get("_AUTH_LATCH"):
                if _auth_recovered():
                    globals()["_AUTH_LATCH"] = False
            _disrupt_eff = bool(_disrupt or globals().get("_AUTH_LATCH"))
            # 고부하 래치(2026-07-26): 수요가 임계 아래로 내려가도 망은 배수 국면 내내 과부하다.
            # 래치 없이 풀면 High가 step42에 가격채널 재활성 → 회복기 urban 캐스케이드로 -65→+188
            # 파국(실측). 교란 래치와 동일하게 밀도 회복 완료 시에만 해제.
            # 회복 판정(2026-07-26 보강): freeway 밀도만 보면 urban 배수가 안 끝났는데 래치가
            # 풀린다(High step46 해제 → -65가 +240으로 파국, 실측). urban 축적도 함께 본다.
            _rec_nu = float(_os.environ.get("AUTH_RECOVER_NU", "400"))
            def _auth_recovered():
                _rc = float(cfg.network.rho_crit)
                if not all((not _d) or max(_d) < _rc for _d in sim.state.freeway_density.values()):
                    return False
                _nu = (float(sim.state.protected_accumulation_veh(cfg.network))
                       + float(sim.state.boundary_in_queue_vehicles(cfg.network)))
                return _nu < _rec_nu
            # peak-seen 래치(2026-07-27): 수요가 한 번이라도 임계를 넘었으면 그 에피소드는
            # '적재(loaded)' 국면으로 취급. 순간수요로 분기하면 웜업·꼬리 구간에 전 셀이
            # '저수요'로 보여 High가 망가진다(실측 -65→+37). Low는 피크가 21778로 임계를
            # 넘지 않아 끝까지 저수요 분기에 남는다.
            if _dem >= _d_lo:
                globals()["_AUTH_PEAK_SEEN"] = True
            _loaded = bool(globals().get("_AUTH_PEAK_SEEN"))
            _high_raw = _dem >= _d_hi
            if _high_raw:
                globals()["_AUTH_HIGH_LATCH"] = True
            elif globals().get("_AUTH_HIGH_LATCH"):
                if _auth_recovered():
                    globals()["_AUTH_HIGH_LATCH"] = False
            _high = bool(_high_raw or globals().get("_AUTH_HIGH_LATCH"))
            globals()["_AUTH_HIGH"] = bool(_high)
            # ── 3-분기 파라미터 (2026-07-27 실측 최적, 감독자X·예산O) ──
            #  교란(래치)  : trust 1.5 · 가격 ON · spillback WF 0            → Inc -47
            #  적재(peak)  : trust 6   · 가격 OFF · WF 0.20 · far_fw 0.5      → Med -97 / Skew -36 / High -1.2
            #  저부하      : trust 6   · 가격 ON  · WF 1.0                    → Low -11.7
            _wf_dis = float(_os.environ.get("AUTH_WF_DISRUPT", "0.0"))
            _wf_load = float(_os.environ.get("AUTH_WF_LOADED", "0.20"))
            _wf_low = float(_os.environ.get("AUTH_WF_LOW", "1.0"))
            _farf_load = float(_os.environ.get("AUTH_FARF_LOADED", "0.5"))
            # budget_fair도 분기 파라미터(2026-07-27 실측): 적재 국면엔 유익(High +2→+0.57),
            # 저부하 국면엔 해로움(Low -11.7→+4.7). 후보 격자 재표본이 저부하선 오히려 오판을 부름.
            if _os.environ.get("AUTH_SMOOTH") == "1":
                # ── 연속 활성화(2026-07-27, 사용자 설계) ──
                # 이산 분기(if/elif)는 분기=시나리오라 룩업테이블에 가깝고 미지 시나리오에 취약.
                # 대신 **상태의 연속 함수**로: 정상 구간엔 거의 0, 심해지면 폭발적으로 활성.
                #   act(s;k) = (exp(k·s) − 1)/(exp(k) − 1)   (k>0 볼록, s∈[0,1])
                # 부하 지수 s_L = 도심 축적(protected+boundary) 정규화, 교란 지수 s_D = 차로손실.
                import math as _m
                _nu_now = (float(sim.state.protected_accumulation_veh(cfg.network))
                           + float(sim.state.boundary_in_queue_vehicles(cfg.network)))
                _nu_lo = float(_os.environ.get("AUTH_NU_LO", "500"))
                _nu_hi = float(_os.environ.get("AUTH_NU_HI", "3000"))
                _k_load = float(_os.environ.get("AUTH_K_LOAD", "5"))
                _k_dis = float(_os.environ.get("AUTH_K_DIS", "5"))
                _dis_ref = float(_os.environ.get("AUTH_DIS_REF", "1.0"))
                _loss = sum(float(v) for _sg in _mfl(list(forecast)).values() for v in _sg.values())
                def _act(_x, _k):
                    _x = min(1.0, max(0.0, _x))
                    return (_m.exp(_k * _x) - 1.0) / (_m.exp(_k) - 1.0) if _k > 1e-9 else _x
                # ★입력 변수(2026-07-27 정정): 부하 지수를 상태량 n_u로 쓰면 **내생 피드백**에
                # 갇힌다 — 컨트롤러가 잘하면 n_u가 낮게 유지되어 활성화가 안 켜지고, 안 켜지니
                # 더 개선 못 함(실측: High에서 a_L이 0.007~0.064 고착, 개입 무산).
                # → 외생 변수인 **예보 총수요**를 기본 입력으로. 컨트롤러가 못 바꾸는 양이라
                # 자기충족 루프가 없다. AUTH_LOAD_SRC=state로 하면 옛 n_u 입력.
                if _os.environ.get("AUTH_LOAD_SRC", "demand") == "state":
                    _sL = (_nu_now - _nu_lo) / max(_nu_hi - _nu_lo, 1.0e-9)
                else:
                    _dm_lo = float(_os.environ.get("AUTH_DEM_LO", "22000"))
                    _dm_hi = float(_os.environ.get("AUTH_DEM_HI", "27000"))
                    _sL = (_dem - _dm_lo) / max(_dm_hi - _dm_lo, 1.0e-9)
                _sD = _loss / max(_dis_ref, 1.0e-9)
                # 교란도 래치(회복 전까지 유지) — 이산 모드와 동일 물리 근거
                if _sD > 0.0:
                    globals()["_AUTH_SD_HOLD"] = max(float(globals().get("_AUTH_SD_HOLD", 0.0)), _sD)
                elif globals().get("_AUTH_SD_HOLD") and _auth_recovered():
                    globals()["_AUTH_SD_HOLD"] = 0.0
                _sD = max(_sD, float(globals().get("_AUTH_SD_HOLD", 0.0)))
                _aL = _act(_sL, _k_load)
                _aD = _act(_sD, _k_dis)
                _trust = _t_big - (_t_big - _t_small) * _aD
                _wf = _wf_low * (1.0 - _aL) * (1.0 - _aD)
                _farf = 1.0 - (1.0 - _farf_load) * _aL
                _prices_on = bool(_aL < float(_os.environ.get("AUTH_PRICE_OFF_AT", "0.5")))
                _bfair = bool(_aL >= float(_os.environ.get("AUTH_BFAIR_AT", "0.5")))
                globals()["_AUTH_ACT"] = (round(_aL, 3), round(_aD, 3), round(_nu_now, 0))
            elif _disrupt_eff:
                _trust, _prices_on, _wf, _farf, _bfair = _t_small, True, _wf_dis, 1.0, False
            elif _loaded:
                _trust, _prices_on, _wf, _farf, _bfair = _t_big, False, _wf_load, _farf_load, True
            else:
                _trust, _prices_on, _wf, _farf, _bfair = _t_big, True, _wf_low, 1.0, False
            cfg.mpc.leader_budget_fair = bool(_bfair)
            for _pa in ("metering_price_enabled", "vsl_price_enabled", "offset_price_enabled"):
                if hasattr(controller, _pa):
                    setattr(controller, _pa, bool(_prices_on))
            cfg.mpc.leader_spillback_freeway_weight = _wf
            cfg.mpc.leader_mfd_far_freeway_weight = _farf
            if hasattr(controller, "signal_price_trust_sec"):
                controller.signal_price_trust_sec = _trust
            _auth_sig = (round(_trust, 2), bool(_prices_on), round(_wf, 2), round(_farf, 2))
            if _auth_sig != globals().get("_AUTH_LAST"):
                globals()["_AUTH_LAST"] = _auth_sig
                print(
                    f"[AUTH_ADAPT] step {step}: demand={_dem:.0f} disrupt={int(_disrupt_eff)} "
                    f"act={globals().get(chr(95)+chr(65)+chr(85)+chr(84)+chr(72)+chr(95)+chr(65)+chr(67)+chr(84))} prices={int(_prices_on)} trust={_trust:.2f} wf={_wf:.2f} farf={_farf:.2f}",
                    flush=True,
                )
        ...
        if step < int(_os.environ.get("WARMUP_NC_STEPS", "0")):
            ...
            # AUTH_ADAPT: 감독자는 **교란 국면에만** 해제. 실측 — SUP는 Low -13/Med -108/Skew -39/
            # High -65로 두루 이득이나 Inc(교란)에선 +739 파국(myopic h3 채점이 회복 포석을 깸).
            # 즉 배제 사유는 "고수요가 아님"이 아니라 "교란 중 근시 스위칭"이다.
            if _os.environ.get("AUTH_ADAPT") == "1" and globals().get("_AUTH_LATCH"):
                _sup_off = True
