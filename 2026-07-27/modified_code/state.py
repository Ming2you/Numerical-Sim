...
class MPCConfig:
    ...
    # Mod1 URBAN_CLF(2026-07-25 자율재설계): urban terminal cost 볼록 CLF 재설계. 기본 off=비트동일.
    leader_urban_clf_enabled: bool = False
    leader_urban_clf_gated: bool = True      # True=far gate 동조(gate셀만), False=always-on
    leader_urban_clf_nref: float = 450.0     # operating point(이 위로 벌점 시작)
    leader_urban_clf_ncrit: float = 550.0    # urban MFD 임계(지나면 production 붕괴)
    leader_urban_clf_njam: float = 1540.0    # urban jam accumulation(7링크×220)
    leader_urban_clf_pmax: float = 640.0     # peak production(drain)
    leader_urban_clf_gmin: float = 60.0      # 최소 drain(gridlock 바닥)
    leader_urban_clf_ttail: float = 0.5      # 기울기 외삽 지평[h]
    ...
    # 대칭 성장-투영 spillback(2026-07-25, 사용자 균형 직관): urban(경계포함)·freeway 성장을 T_lead
    # 선형 투영해 지연 spillback 선반영. gating으로 못속임(경계 포함)·대칭(균형). 기본 off=비트동일.
    leader_spillback_enabled: bool = False
    leader_spillback_weight: float = 1.0         # 전체 스케일
    leader_spillback_urban_weight: float = 1.0   # ★균형 calibration(urban 몫)
    leader_spillback_freeway_weight: float = 1.0 # ★균형 calibration(freeway 몫)
    leader_spillback_lead_h: float = 0.5         # 성장 투영 지평[h]
    leader_spillback_nref_urban: float = 500.0   # urban 정상 임계(이 위 excess만)
    leader_spillback_nref_freeway: float = 0.0   # freeway는 over-critical이 이미 excess
    # control-move penalty(2026-07-25): budget 급변(슬램) 벌점 → 매끈(오라클 모사). 기본 off=비트동일.
    leader_move_enabled: bool = False
    leader_move_weight: float = 1.0
    leader_move_nuf_weight: float = 1.0          # |ΔN_UF| 가중
    leader_move_np_weight: float = 0.0           # |ΔN_P| 가중(기본 off, N_UF만 먼저)
