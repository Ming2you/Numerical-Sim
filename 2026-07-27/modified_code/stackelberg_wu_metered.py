...
class StackelbergWuMeteredController(StackelbergMPCController):
    ...
    def decide_with_info(
        ...
        if isinstance(follower_ls, WuFaithfulFollower):
            ...
            elif self.nuf_link_share_mode == "price":
                # PRICE-SHARE(2026-07-26, 사용자 설계): 링크 예산 배분을 상태 휴리스틱이 아니라
                # **한계 외부성 가격**으로 결정한다. 기존 density(ω∝headroom)는 스텝 시작에
                # 확정되어 가격이 링크 간 물량을 못 옮기고(구조적 무권한), 실측상 가격은
                # 차등(변동계수 0.63)인데 실제 배분은 균일(0.22)이라 정보가 3배 소실됐다.
                # ω_l ∝ exp(−β·p̄_l): 한계가격이 비싼 링크(=방류의 외부비용이 큰 곳)에 예산을
                # 덜 준다. 가격은 직전 스텝 값(ω는 가격 갱신 전에 필요) — 1스텝 지연 수용.
                follower_ls._wu._omega_f = self._link_share_omega_price(state, follower_ls)
            else:
    ...
    def _link_share_omega_price(self, state: TrafficState, follower) -> Dict[str, float]:
        """가격 기반 link budget 분할 — ω_l ∝ exp(−β·p̄_l), p̄_l = 링크 l 소속 램프 한계가격 평균.

        Stackelberg 가격 메커니즘의 정도(正道): 예산을 없애지 않고, **배분을 가격이 하게** 한다.
        가격이 없거나 전부 0이면 density 모드로 안전 폴백(비트동일 경로 보존).
        β=LINK_SHARE_BETA(기본 200) — 가격 스케일이 0.001~0.01이라 지수가 유의미해지는 크기."""
        import os as _o
        net = self.cfg.network
        links = list(net.freeway_links)
        n_l = max(len(links), 1)
        prices = getattr(follower, "metering_marginal_price", None)
        if not prices:
            return self._link_share_omega(state)
        agg: Dict[str, float] = {l: 0.0 for l in links}
        cnt: Dict[str, int] = {l: 0 for l in links}
        for ramp, pv in prices.items():
            link = net.ramp_to_freeway.get(ramp)
            if link in agg:
                agg[link] += abs(float(pv))
                cnt[link] += 1
        pbar = {l: (agg[l] / cnt[l] if cnt[l] else 0.0) for l in links}
        if max(pbar.values(), default=0.0) <= 1.0e-12:
            return self._link_share_omega(state)
        beta = float(_o.environ.get("LINK_SHARE_BETA", "200"))
        ref = min(pbar.values())
        w = {l: __import__("math").exp(-beta * (pbar[l] - ref)) for l in links}
        tot = sum(w.values())
        if tot <= 1.0e-12:
            return {l: 1.0 / n_l for l in links}
