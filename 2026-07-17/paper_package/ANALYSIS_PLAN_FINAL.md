...
2. **선형 가격 × 이산 후보 = bang-bang**: 내부 rung 0/160, 부호→끝점 80~85%,
   ...
   **세 번째 두더지 잡기**(PD4: skew↔190, 박스: 170s↔200, cross: 200↔190) — 극단 부하
   셀은 단일 처방으로 동시 구제 불가. 끝점 선택은 박스 내에서 강건.
   **2차 자기항도 사전 기각(2026-07-17 밤, 구현 0)**: 기존 fd3 3점 진단
   (`wu_b3_meter_fd3_{ramp}_{lo,base,hi}`)으로 곡률을 직접 측정 — 볼록 61%(동전 수준),
   D2 부호 반전율 31%(≈3스텝마다 뒤집힘 = 잡음), 표적 셀 200_w의 E램프 2개는 75%+
   **오목**(2차항이 끝점 선택을 오히려 강화). 비용곡선의 실구조는 곡률이 아니라
   **문턱-절벽**(fd3의 설계 목적인 cliffup이 그 증거) — 절벽은 B3CERT 인증 게이트 소관.
   ⇒ 탈선형화 사다리 종결: 박스(외삽 제거)✓ walk(시야)✓ cross✗ 가중✗ 2차항✗.
