...
| P-CENT | `CentralizedMPC(mode="proposed")` — 전권 joint(green+VSL+offset+metering+게이트 서비스 분율). 실런 솔버 = **centralized structured grid search(coarse→fine, 전권 joint 행동공간), serial(단일 프로세스 직렬)**. spec 16.13의 "동일 budget centralized numerical reference"이며, 제어주기 180 s를 큰 여유로 하회하는 실시간 가능한 중앙집중 참조다. 이동 한계 의도적 미부과(rate-limit-free 상한 프레이밍, 사용자 결정 2026-07-18; VSL만 명목 max_vsl_step=20 bound) | runner:445–447; cm:59–77(권한), 103–104(VSL bound), 437–501(coarse→fine grid); _paper_pcent run_log 진단 |